...
func _process(_delta: float) -> void:
	if first_card.get_child_count() == 0 and second_card.get_child_count() == 1:
		var card: Node2D = second_card.get_child(0)
	...
	if second_card.get_child_count() == 0 and reserve_cards.get_child_count() > 0:
		var card: Node2D = reserve_cards.get_child(reserve_cards.get_child_count() - 1)
...
func move_first_to_second() -> void:
	var old_first_card: Node2D = first_card.get_child(0) if first_card.get_child_count() > 0 else null
...
func move_second_to_reserve() -> void:
	var old_second_card: Node2D = second_card.get_child(0) if second_card.get_child_count() > 0 else null
	if old_second_card != null:
		second_card.remove_child(old_second_card)
		reserve_cards.add_child(old_second_card)
		
func move_second_to_first() -> void:
	var old_second_card: Node2D = second_card.get_child(0) if second_card.get_child_count() > 0 else null
	if old_second_card != null:
		second_card.remove_child(old_second_card)
...
func move_reserve_to_second() -> void:
	var old_reserve_card: Node2D = reserve_cards.get_child(reserve_cards.get_child_count()-1) if reserve_cards.get_child_count() > 0 else null

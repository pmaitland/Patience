...
func can_pile_in_depot(card: Node2D) -> bool:
	return card.depot != null and card.value == value + 1 and (card.suit == (suit + 1) % 4 or card.suit == (suit + 3) % 4)
	
func can_pile_in_foundation(card: Node2D) -> bool:
	return card.foundation != null and child_card == null and card.value == value - 1 and card.suit == suit
